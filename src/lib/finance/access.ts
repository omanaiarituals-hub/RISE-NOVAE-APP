 ...
 *
 * - En développement :
 *   FINANCE_DEV_PREVIEW=true autorise l'aperçu local,
 *   même si la session Supabase serveur n'est pas encore disponible.
 *
 * - En production :
 *   seuls les UUID Supabase présents dans
 *   FINANCE_PRIVATE_BETA_USER_IDS sont autorisés.
 */
export function isFinanceBetaAllowed(
  userId: string | null | undefined
): boolean {
  if (
    process.env.NODE_ENV !== 'production' &&
    process.env.FINANCE_DEV_PREVIEW === 'true'
  ) {
    return true
  }
