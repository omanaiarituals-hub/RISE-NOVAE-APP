...
export default function FinanceTransactionList({ items }: { items: Item[] }) {
  ...
  return <section className="rounded-[28px] border border-[var(--novae-border)] bg-[var(--novae-surface)] p-4 sm:p-6">
    <div className="flex flex-wrap items-end justify-between gap-3">
      ...
      <div className="flex flex-wrap gap-2"><span className="rounded-full border border-[var(--novae-border)] px-3 py-2 text-xs font-bold">{items.length} opération(s)</span><button onClick={() => void analyse()} disabled={running || items.length === 0} className="rounded-full bg-[var(--novae-primary)] px-4 py-2 text-xs font-black text-white disabled:cursor-not-allowed disabled:opacity-50">{running ? 'Analyse…' : 'Analyser les opérations'}</button></div>
    </div>
    {message && <div className="mt-4 rounded-2xl bg-black/[.04] p-3 text-sm">{message}</div>}
    {!items.length ? <div className="mt-6 rounded-2xl border border-dashed border-[var(--novae-border)] bg-[var(--novae-background)] p-5 text-sm text-[var(--novae-text-muted)]"><p className="font-bold text-[var(--novae-text-main)]">Aucune opération à analyser.</p><p className="mt-1">Connecte ou synchronise un compte bancaire inclus dans NOVAÉ.</p><Link href="/finances/banking" className="mt-3 inline-flex rounded-full border border-[var(--novae-border)] bg-[var(--novae-surface)] px-4 py-2 text-xs font-black text-[var(--novae-text-main)] no-underline">Gérer mes comptes →</Link></div> : <div className="mt-6 overflow-hidden rounded-2xl border border-[var(--novae-border)]">{items.map((item, index) => <Link href={`/finances/transactions/${item.id}`} key={item.id} className={`grid grid-cols-[56px_1fr_auto] gap-3 p-4 transition hover:bg-black/[.025] ${index ? 'border-t border-[var(--novae-border)]' : ''}`}>
