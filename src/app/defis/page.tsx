  ...
  return (
    <div style={{ marginTop: 10 }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4 }}>
        <span style={{ fontSize: 11, color: C.gris, fontFamily: "'DM Sans',sans-serif" }}>
          {doneDays}/{defi.duration} jours complétés
        </span>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
        {defi.days.map(d => (
          <button key={d.day} onClick={() => onToggleDay(defi.id, d.day)}
            title={`Jour ${d.day}`}
            style={{
              width: 32, height: 32, borderRadius: 8,
              background: d.done ? cat.border : C.grisClair,
              border: `1.5px solid ${d.done ? cat.border : C.grisClair}`,
              cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center",
              fontSize: 10, color: d.done ? C.blanc : C.gris,
...
function DefiCard({ defi, onStart, onToggleDay, onComplete, onAbandon, onShare }: DefiCardProps) {
  ...
  return (
    <div style={{
      background: isTermine ? "#F5FBF5" : (isAbandonne ? "#F8F8F8" : C.blanc),
      border: `1.5px solid ${isTermine ? "#90C8A8" : (isEnCours ? cat.border : C.grisClair)}`,
      borderRadius: 14, padding: "16px 18px", marginBottom: 12,
      opacity: isAbandonne ? 0.6 : 1,
      boxShadow: isEnCours ? `0 2px 12px ${cat.border}44` : "none",
    }}>
      {/* Header */}
      ...
      {/* Day checks — only when en cours or termine */}
      {(isEnCours || isTermine) && (
        <DayChecks defi={defi} onToggleDay={onToggleDay} />
      )}

      {/* Actions */}
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 12 }}>
        {isDisponible && (
          <button onClick={() => onStart(defi.id)}
            style={{ padding: "8px 18px", borderRadius: 8, border: "none", background: cat.border, color: C.blanc, cursor: "pointer", fontFamily: "'DM Sans',sans-serif", fontSize: 12, fontWeight: 600 }}>
            ▶ Commencer
          </button>
        )}
        {isEnCours && allDone && (
          <button onClick={() => onComplete(defi.id)}
            style={{ padding: "8px 18px", borderRadius: 8, border: "none", background: "#7BAF8E", color: C.blanc, cursor: "pointer", fontFamily: "'DM Sans',sans-serif", fontSize: 12, fontWeight: 600 }}>
            🏆 Valider le défi
          </button>
        )}
        {isEnCours && (
          <button onClick={() => onAbandon(defi.id)}
            style={{ padding: "8px 14px", borderRadius: 8, border: `1px solid ${C.grisClair}`, background: C.blanc, cursor: "pointer", fontFamily: "'DM Sans',sans-serif", fontSize: 11, color: C.gris }}>
            Abandonner
          </button>
        )}
        {isTermine && (
          <button onClick={() => onShare(defi)}
            style={{ padding: "8px 18px", borderRadius: 8, border: "none", background: "#E8D080", color: "#7A6010", cursor: "pointer", fontFamily: "'DM Sans',sans-serif", fontSize: 12, fontWeight: 600 }}>
            📢 Partager
          </button>
        )}
      </div>
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.4)", zIndex: 100, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
      <div style={{ background: C.blanc, borderRadius: 16, padding: 24, width: "100%", maxWidth: 420, boxShadow: "0 8px 40px rgba(0,0,0,0.15)", maxHeight: "90vh", overflowY: "auto" }}>
        ...
        <textarea value={description} onChange={e => setDescription(e.target.value)} placeholder="Décris ton défi…"
          style={{ ...inputStyle(), minHeight: 80, resize: "vertical", marginBottom: 12 } as React.CSSProperties} />
          ...
          <div style={{ display: "flex", gap: 20, padding: "10px 0 14px", flexWrap: "wrap" }}>
            {[{ icon: "⚡", val: enCours, label: "En cours", color: C.noir }, { icon: "🏆", val: termines, label: "Accomplis", color: C.noir }, { icon: "✨", val: totalPoints, label: "Points", color: C.roseDark }].map((s, i) => (
              <div key={i} style={{ display: "flex", alignItems: "center", gap: 6 }}>
                ...
                <div><div style={{ fontSize: 16, fontWeight: 700, color: s.color, fontFamily: "'Cormorant Garamond',serif" }}>{s.val}</div><div style={{ fontSize: 10, color: C.gris, textTransform: "uppercase", letterSpacing: 0.5 }}>{s.label}</div></div>
              </div>
            ))}
            {badges.length > 0 && (
