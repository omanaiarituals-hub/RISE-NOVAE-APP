...
export async function GET(request: NextRequest) {
  ...
  // L'analyse visible doit suivre exactement les comptes actuellement inclus
  // dans NOVAÉ. Les anciennes données sandbox restent hors calcul.
  ...
  if (activeAccountError) {
    return NextResponse.json(
      { error: 'active_accounts_load_failed', detail: activeAccountError.message },
      { status: 500 },
    )
  }
  ...
  if (activeAccountIds.length) {
    ...
    if (activeTxError) {
      return NextResponse.json(
        { error: 'active_transactions_load_failed', detail: activeTxError.message },
        { status: 500 },
      )
    }
  ...
  if (activeTransactionIds.length) {
    ...
    if (error) {
      return NextResponse.json(
        { error: 'annotations_load_failed', detail: error.message },
        { status: 500 },
      )
    }
  ...
  const [{ data: recurring, error: recurringError }, { data: insights, error: insightError }] =
    await Promise.all([
      supabaseAdmin
        ...
        .select('id,name,commitment_type,amount,frequency,next_due_date,confidence')
        .eq('user_id', identity.id)
  ...
  if (recurringError || insightError) {
    return NextResponse.json(
      {
        error: 'intelligence_load_failed',
        detail: recurringError?.message || insightError?.message,
      },
      { status: 500 },
    )
  }
  ...
  if (ids.length) {
    const { data, error } = await supabaseAdmin
      .from('finance_transactions')
      .select('id,transaction_date,amount,currency,raw_label,merchant_name')
      .eq('user_id', identity.id)
      .in('id', ids)

    if (error) {
      return NextResponse.json(
        { error: 'intelligence_transactions_load_failed', detail: error.message },
        { status: 500 },
      )
    }
  ...
  return NextResponse.json({
    ...
    needs_analysis: activeTransactionIds.length > 0 && total === 0,
    active_transactions: activeTransactionIds.length,
  })
}
