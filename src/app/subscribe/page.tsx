...
export default function SubscribePage() {
  ...
  const [userData, setUserData] = useState<{ subscription_tier?: string; trial_ends_at?: string } | null>(null)
  ...
  const isInTrial = userData?.subscription_tier === 'trial' &&
                    userData?.trial_ends_at &&
                    new Date(userData.trial_ends_at) > now

  const trialEndsDate = userData?.trial_ends_at ? new Date(userData.trial_ends_at) : null
  const trialEndsFormatted = trialEndsDate
    ? trialEndsDate.toLocaleDateString('fr-FR', { day: 'numeric', month: 'long', year: 'numeric' })
    : ''

  let statusText = ''
  if (isPremium) statusText = '✦ Tu es déjà Premium'
  else if (isInTrial && isAvantPremiere) statusText = `✨ Avant-première gratuite jusqu'au ${trialEndsFormatted}`
  else if (isInTrial) statusText = `✨ Essai gratuit jusqu'au ${trialEndsFormatted}`
  else if (isAvantPremiere) statusText = `✨ Avant-première gratuite jusqu'au 1er juin 2026`
  ...
  if (pageLoading) {
    return (
      <div style={{ minHeight: '100vh', background: C.cream, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <p style={{ color: C.brownLight, fontFamily: "'DM Sans', sans-serif", fontSize: 14 }}>Chargement…</p>
      </div>
    )
  }
      ...
      <div style={{ minHeight: '100vh', fontFamily: "'DM Sans', sans-serif", color: C.brown, position: 'relative', zIndex: 2 }}>
        ...
        <div style={{ maxWidth: 720, margin: '0 auto', padding: '40px 20px 80px' }}>
          ...
          <div style={{ textAlign: 'center', marginBottom: 24 }}>
            <p style={{ color: C.copperDark, letterSpacing: '0.25em', fontSize: 11, marginBottom: 14, fontWeight: 600, textTransform: 'uppercase' }}>
              ✦ Accès Premium
            </p>
            <h1 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 42, color: C.brown, margin: '0 0 10px', fontWeight: 500, lineHeight: 1.1 }}>
              NOVAÉ Premium
            </h1>
            <p style={{ color: C.brownLight, fontSize: 16, lineHeight: 1.6, margin: '0 auto', maxWidth: 440, fontStyle: 'italic' }}>
          ...
          <div style={{ textAlign: 'center', marginBottom: 28 }}>
            <span style={{
              ...
              background: isPremium ? 'linear-gradient(135deg, rgba(123,168,105,0.18), rgba(123,168,105,0.08))' : 'linear-gradient(135deg, rgba(196,149,106,0.22), rgba(196,149,106,0.08))',
              border: `1px solid ${isPremium ? 'rgba(123,168,105,0.4)' : 'rgba(196,149,106,0.4)'}`,
              borderRadius: 24, fontSize: 12, fontWeight: 600,
              color: isPremium ? C.green : C.copperDark, letterSpacing: '0.05em',
            }}>
              {statusText}
            </span>
          </div>
          ...
          <div style={{
            ...
            <div style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 64, fontWeight: 500, color: C.copperDark, lineHeight: 1, marginBottom: 4 }}>
              7,90€
            </div>
            ...
            <div style={{ height: 1, background: 'rgba(212,165,116,0.25)', margin: '20px auto', maxWidth: 200 }} />

            <p style={{ fontSize: 13, color: C.brownLight, lineHeight: 1.65, marginBottom: 24, maxWidth: 420, margin: '0 auto 24px' }}>
              {isPremium
            ...
            {isPremium ? (
              <Link href="/" style={{
                ...
                fontFamily: 'inherit', boxShadow: '0 6px 16px rgba(123, 168, 105, 0.25)',
              ...
              <button
                ...
                style={{
                  ...
                  background: loading ? 'rgba(196,149,106,0.4)' : 'linear-gradient(135deg, #c4956a, #8b5a3c)',
                  color: '#fff', border: 'none', borderRadius: 12,
            ...
            <p style={{ fontSize: 11, color: C.brownLight, marginTop: 14, opacity: 0.7 }}>
          ...
          <div style={{
            ...
            <h2 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 24, color: C.brown, margin: '0 0 18px', fontWeight: 500, textAlign: 'center' }}>
              Ce que tu débloques
            </h2>
            <div style={{ display: 'grid', gap: 14 }}>
              {PREMIUM_FEATURES.map((feat, i) => (
                <div key={i} style={{ display: 'flex', gap: 14, alignItems: 'flex-start' }}>
                  <div style={{ fontSize: 24, flexShrink: 0, lineHeight: 1, marginTop: 2 }}>{feat.emoji}</div>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 14, fontWeight: 700, color: C.brown, marginBottom: 2 }}>
                      {feat.title}
                    </div>
          ...
          <div style={{
            ...
            <h2 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 24, color: C.brown, margin: '0 0 6px', fontWeight: 500, textAlign: 'center' }}>
              Free vs Premium
            </h2>
            <p style={{ fontSize: 12, color: C.brownLight, textAlign: 'center', marginBottom: 18, fontStyle: 'italic' }}>
              Tout ce qui change selon ton plan
            </p>

            <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) 60px 60px', padding: '8px 0', fontSize: 10, fontWeight: 700, color: C.brownLight, textTransform: 'uppercase', letterSpacing: '0.08em' }}>
              <div>Module</div>
              <div style={{ textAlign: 'center' }}>Free</div>
              <div style={{ textAlign: 'center', color: C.copperDark }}>Premium</div>
            </div>
            ...
            {COMPARISON.map((row, i) => {
              if (row.isHeader) {
                return (
                  <div key={i} style={{
                    ...
                    color: C.copperDark, textTransform: 'uppercase', letterSpacing: '0.15em',
              ...
              return (
                <div key={i} style={{
                  display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) 60px 60px',
                  padding: '10px 0', borderTop: '1px solid rgba(212,165,116,0.12)',
                  fontSize: 12, alignItems: 'center',
                }}>
                  <div style={{ color: C.brown }}>{row.label}</div>
                  <div style={{ textAlign: 'center', fontWeight: 600, color: row.free === '✓' ? C.green : row.free === '🔒' ? C.brownLight : C.copperDark }}>{row.free}</div>
                  <div style={{ textAlign: 'center', fontWeight: 600, color: C.green }}>{row.premium|| ''}</div>
                </div>
              )
            })}
          </div>
          ...
          <div style={{ textAlign: 'center', marginTop: 28 }}>
            <p style={{ fontSize: 12, color: C.brownLight, lineHeight: 1.8, opacity: 0.8 }}>
              ✦ Sans engagement · Résiliable à tout moment depuis tes paramètres<br/>
              🔒 Paiement sécurisé par Stripe · Tes informations bancaires ne sont jamais stockées par NOVAÉ
            </p>
          </div>
