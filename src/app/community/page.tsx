...
export default function CommunityPage() {
  ...
  const loadPosts = async () => {
    ...
    setPosts(postsData.map(p => ({ ...p, pseudo: pseudoMap[p.user_id] || p.user_id.slice(0, 8), liked_by_me: likedIds.has(p.id) })))
  }
  ...
  const loadRanking = async () => {
    ...
    setRanking(sorted.map(([userId, count], index) => ({ rank: index + 1, user_id: userId, pseudo: pseudoMap[userId] || userId.slice(0, 8), count, isMe: userId === user?.id })))
  }
  ...
  const loadComments = async (postId: string) => {
    ...
    const pseudoMap: Record<string, string> = {}
    profiles?.forEach(p => { pseudoMap[p.user_id] = p.pseudo })
    setComments(prev => ({ ...prev, [postId]: data.map(c => ({ ...c, pseudo: pseudoMap[c.user_id] || c.user_id.slice(0, 8) })) }))
  }
                ...
                posts.map(post => (
                  <div key={post.id} className="bg-white rounded-2xl border border-novae-beige/20 shadow-sm overflow-hidden">
                    ...
                    <div className="px-4 pb-3 flex items-center gap-4 border-t border-novae-beige/10 pt-2">
                      <button onClick={() => handleLike(post)} className={`flex items-center gap-1.5 text-xs transition-colors ${post.liked_by_me ? 'text-novae-gold' : 'text-novae-anthracite/40 hover:text-novae-gold'}`}>
                        <span>{post.liked_by_me ? '💛' : '🤍'}</span>
                        <span>{post.likes_count > 0 ? post.likes_count : ''}</span>
                      </button>
                      <button onClick={() => toggleComments(post.id)} className="flex items-center gap-1.5 text-xs text-novae-anthracite/40 hover:text-novae-anthracite transition-colors">
                        <span>💬</span>
              ...
              {ranking.length === 0 ? (
                ...
                <div className="bg-white rounded-2xl border border-novae-beige/20 shadow-sm overflow-hidden">
                  {ranking.map((entry, index) => (
                    <div key={entry.user_id} className={`flex items-center gap-3 px-4 py-3 ${index < ranking.length - 1 ? 'border-b border-novae-beige/10' : ''} ${entry.isMe ? 'bg-novae-gold/5' : ''}`}>
