...
const FALLBACK: Record<number, string> = {
  1:  'https://images.unsplash.com/photo-1516912481808-3406841bd33c?w=1200&q=75',
  2:  'https://images.unsplash.com/photo-1485236715568-ddc5ee6ca227?w=1200&q=75',
  3:  'https://images.unsplash.com/photo-1462275646964-a0e3386b89fa?w=1200&q=75',
  4:  'https://images.unsplash.com/photo-1518173946687-a4c8892bbd9f?w=1200&q=75',
  5:  'https://images.unsplash.com/photo-1490750967868-88df5691cc11?w=1200&q=75',
  6:  'https://images.unsplash.com/photo-1507525428034-b723cf961d3e?w=1200&q=75',
  7:  'https://images.unsplash.com/photo-1504701954957-2010ec3bcec1?w=1200&q=75',
  8:  'https://images.unsplash.com/photo-1500534314209-a25ddb2bd429?w=1200&q=75',
  9:  'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=1200&q=75',
  10: 'https://images.unsplash.com/photo-1476820865390-c52aeebb9891?w=1200&q=75',
  11: 'https://images.unsplash.com/photo-1482192505345-5852583c8e98?w=1200&q=75',
  12: 'https://images.unsplash.com/photo-1418985991508-e47386d96a71?w=1200&q=75',
}
  ...
  useEffect(() => {
    ...
    if (cached) {
      try {
        ...
        if (d.month === month && d.url) { setBgUrl(d.url); return }
      } catch {}
    }
    ;(async () => {
      try {
        ...
        localStorage.setItem(key, JSON.stringify({ url, month }))
        setBgUrl(url)
      } catch {
        setBgUrl(FALLBACK[month] || FALLBACK[4])
      }
    })()
  }, [])
  return bgUrl
...
export default function HomePage() {
  ...
  const loadData = async () => {
    ...
    if (prog.data) { const d = prog.data.current_day || 1; setCurrentDay(d); setProgramProgress(Math.round((d/90)*100)); setStreak(prog.data.streak||0) }
    const t = tasks.data || []; setTodayTasks(t); setTodayTasksDone(t.filter((x:any) => x.status==='completed').length)
    const r = routines.data || []; setRoutinesTotal(r.length); setRoutinesDone(r.filter((x:any) => x.completed).length)
    try {
  ...
  // Styles glassmorphism réutilisables
  const glass = { background: 'rgba(15,12,10,0.72)', backdropFilter: 'blur(20px)', border: '1px solid rgba(255,255,255,0.07)' }

  return (
    <>
      <OnboardingTour forceShow={showTour} onClose={() => setShowTour(false)} />

      {/* ── FOND D'ÉCRAN SAISONNIER (fixed, derrière tout) ── */}
      <div style={{
        position: 'fixed', inset: 0, zIndex: 0,
        background: bgUrl ? `url(${bgUrl}) center/cover no-repeat fixed` : '#0F0C0A',
        filter: 'brightness(0.42) saturate(0.65)',
        transition: 'opacity 1.2s ease',
      }} />
      {/* Voile sombre pour lisibilité */}
      <div style={{ position: 'fixed', inset: 0, zIndex: 1, background: 'rgba(10,8,6,0.5)' }} />
      ...
      <div style={{ minHeight: '100vh', fontFamily: "'DM Sans', sans-serif", position: 'relative', zIndex: 2 }}>
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            {currentDay > 0 && (
              <span style={{ fontSize: 11, color: '#C4956A', background: 'rgba(196,149,106,0.15)', padding: '2px 10px', borderRadius: 20, fontWeight: 500 }}>
                Jour {currentDay}/90
              </span>
            )}
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            {newCommunityPosts > 0 && (
              <Link href="/community" onClick={() => localStorage.setItem('novae-community-last-visit', new Date().toISOString())}
                style={{ textDecoration: 'none', display: 'flex', alignItems: 'center', gap: 5, background: 'rgba(245,208,220,0.12)', border: '1px solid rgba(224,160,184,0.25)', borderRadius: 20, padding: '4px 10px' }}>
                <span style={{ fontSize: 13 }}>💬</span>
                <span style={{ fontSize: 11, fontWeight: 700, color: '#F0B8C8' }}>{newCommunityPosts} message{newCommunityPosts > 1 ? 's' : ''}</span>
              </Link>
            )}
            {user ? <UserMenu /> : (
              <Link href="/auth" style={{ padding: '8px 20px', borderRadius: 20, border: '1.5px solid rgba(212,160,144,0.4)', background: 'rgba(255,255,255,0.05)', color: '#D4A090', textDecoration: 'none', fontSize: 13, fontWeight: 600 }}>
                Se connecter
              </Link>
            )}
          </div>
        ...
        <main style={{ maxWidth: 640, margin: '0 auto', padding: '24px 20px 120px' }}>
          ...
          <div style={{ marginBottom: 24 }}>
            ...
            <h1 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 36, fontWeight: 600, color: '#FFFFFF', margin: '0 0 6px', lineHeight: 1.15 }}>
              {greeting}{pseudo ? `, ${pseudo}` : ''} 👋
            </h1>
            {currentDay > 0 && (
              <p style={{ fontSize: 14, color: 'rgba(255,255,255,0.45)', margin: 0, fontStyle: 'italic' }}>{dailyMessage}</p>
            )}
          </div>
          ...
          {user && currentDay > 0 && (
            <Link href="/program" style={{ textDecoration: 'none', display: 'block', marginBottom: 16 }}>
              <div style={{ ...glass, borderRadius: 20, padding: '24px', position: 'relative', overflow: 'hidden', boxShadow: '0 8px 40px rgba(0,0,0,0.35)' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16 }}>
                  <div>
                    <span style={{ fontSize: 11, fontWeight: 600, letterSpacing: '0.15em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.35)', display: 'block', marginBottom: 4 }}>{phaseInfo.label}</span>
                    <div style={{ display: 'flex', alignItems: 'baseline', gap: 6 }}>
                      ...
                      <span style={{ fontSize: 18, color: 'rgba(255,255,255,0.25)', fontWeight: 300 }}>/90</span>
                    </div>
                  </div>
                  <div style={{ textAlign: 'right' }}>
                    {streak > 0 && (
                      <div style={{ background: 'rgba(255,165,0,0.1)', border: '1px solid rgba(255,165,0,0.2)', borderRadius: 10, padding: '6px 12px', marginBottom: 8 }}>
                        <span style={{ fontSize: 14 }}>🔥</span>
                        <span style={{ fontSize: 12, color: '#FFA500', fontWeight: 600, marginLeft: 4 }}>{streak} jours</span>
                      </div>
                    )}
                    <span style={{ fontSize: 11, color: 'rgba(255,255,255,0.25)' }}>{programProgress}% accompli</span>
                  </div>
                </div>
                <div style={{ background: 'rgba(255,255,255,0.07)', borderRadius: 4, height: 4, marginBottom: 14, overflow: 'hidden' }}>
                ...
                <p style={{ fontSize: 13, color: 'rgba(255,255,255,0.45)', margin: '0 0 16px', lineHeight: 1.5 }}>{phaseInfo.message}</p>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10, marginBottom: 16 }}>
            <Link href="/planner" style={{ textDecoration: 'none' }}>
              <div style={{ ...glass, borderRadius: 14, padding: '14px 12px', textAlign: 'center' }}>
                <span style={{ fontSize: 22, display: 'block', marginBottom: 6 }}>📋</span>
                <div style={{ fontSize: 20, fontWeight: 700, color: '#FFFFFF', fontFamily: "'Cormorant Garamond', serif", lineHeight: 1 }}>{todayTasksDone}/{todayTasks.length || 0}</div>
                <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.35)', marginTop: 3, fontWeight: 500 }}>tâches</div>
                {todayTasks.length > 0 && todayTasksDone === todayTasks.length && <div style={{ fontSize: 10, color: '#4CAF50', marginTop: 4, fontWeight: 600 }}>✓ Tout fait !</div>}
              </div>
            </Link>
            <Link href="/routines" style={{ textDecoration: 'none' }}>
              <div style={{ ...glass, borderRadius: 14, padding: '14px 12px', textAlign: 'center' }}>
                <span style={{ fontSize: 22, display: 'block', marginBottom: 6 }}>☀️</span>
                <div style={{ fontSize: 20, fontWeight: 700, color: '#FFFFFF', fontFamily: "'Cormorant Garamond', serif", lineHeight: 1 }}>{routinesDone}/{routinesTotal || 0}</div>
                <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.35)', marginTop: 3, fontWeight: 500 }}>routines</div>
                {routinesTotal > 0 && routinesDone === routinesTotal && <div style={{ fontSize: 10, color: '#4CAF50', marginTop: 4, fontWeight: 600 }}>✓ Parfait !</div>}
              </div>
            </Link>
            <Link href="/agent" style={{ textDecoration: 'none' }}>
              <div style={{ background: 'rgba(196,149,106,0.14)', backdropFilter: 'blur(20px)', border: '1px solid rgba(196,149,106,0.22)', borderRadius: 14, padding: '14px 12px', textAlign: 'center' }}>
                <span style={{ fontSize: 22, display: 'block', marginBottom: 6 }}>🤖</span>
                <div style={{ fontSize: 12, fontWeight: 600, color: '#C4956A', lineHeight: 1.2 }}>Agent</div>
                <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.35)', marginTop: 3, fontWeight: 500 }}>NOVAÉ</div>
          ...
          {intention ? (
            <div style={{ ...glass, borderRadius: 14, padding: '14px 18px', marginBottom: 16, display: 'flex', alignItems: 'center', gap: 12 }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <p style={{ fontSize: 15, color: 'rgba(255,255,255,0.85)', fontStyle: 'italic', margin: 0, fontFamily: "'Cormorant Garamond', serif" }}>"{intention}"</p>
              </div>
            ...
            <Link href="/routines" style={{ textDecoration: 'none', display: 'block', marginBottom: 16 }}>
              <div style={{ background: 'rgba(196,149,106,0.08)', backdropFilter: 'blur(20px)', border: '1px dashed rgba(196,149,106,0.28)', borderRadius: 14, padding: '14px 18px', display: 'flex', alignItems: 'center', gap: 12 }}>
                <span style={{ fontSize: 20 }}>✨</span>
                <div style={{ flex: 1 }}>
                  <p style={{ fontSize: 13, fontWeight: 600, color: '#C4956A', margin: '0 0 2px' }}>Définir mon intention du jour</p>
                  <p style={{ fontSize: 11, color: 'rgba(255,255,255,0.35)', margin: 0 }}>Commence avec clarté et focus</p>
                </div>
          ...
          {newCommunityPosts > 0 && (
            <Link href="/community" style={{ textDecoration: 'none', display: 'block', marginBottom: 16 }}
              ...
              <div style={{ background: 'rgba(245,208,220,0.1)', backdropFilter: 'blur(20px)', border: '1px solid rgba(224,160,184,0.2)', borderRadius: 14, padding: '14px 18px', display: 'flex', alignItems: 'center', gap: 12 }}>
                <div style={{ width: 42, height: 42, borderRadius: 12, background: 'rgba(224,160,184,0.15)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20, flexShrink: 0 }}>💬</div>
                <div style={{ flex: 1 }}>
                  <p style={{ margin: 0, fontSize: 13, fontWeight: 700, color: 'white' }}>Communauté NOVAÉ</p>
                  <p style={{ margin: '2px 0 0', fontSize: 12, color: 'rgba(255,255,255,0.35)' }}>{newCommunityPosts} nouveau{newCommunityPosts > 1 ? 'x' : ''} message{newCommunityPosts > 1 ? 's' : ''} depuis ta dernière visite</p>
                </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
            ...
            <button onClick={() => setShowModules(!showModules)}
              style={{ fontSize: 11, color: 'rgba(255,255,255,0.45)', background: 'rgba(255,255,255,0.05)', backdropFilter: 'blur(8px)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 20, padding: '5px 14px', cursor: 'pointer', fontFamily: "'DM Sans', sans-serif", display: 'flex', alignItems: 'center', gap: 6, whiteSpace: 'nowrap' }}>
              ...
              {modules.map(mod => (
                <Link key={mod.href} href={mod.href} style={{ textDecoration: 'none' }}>
                  <div style={{ ...glass, borderRadius: 12, padding: '12px 14px', display: 'flex', alignItems: 'center', gap: 10 }}>
                    ...
                    <span style={{ fontSize: 12, fontWeight: 500, color: 'rgba(255,255,255,0.8)' }}>{mod.title}</span>
                  </div>
                </Link>
              ))}
            </div>
        ...
        <div style={{ position: 'fixed', bottom: 70, right: 16, zIndex: 50, display: 'flex', flexDirection: 'column', gap: 8 }}>
          <button onClick={restartTour} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '8px 14px', borderRadius: 20, background: 'rgba(15,12,10,0.85)', backdropFilter: 'blur(12px)', border: '1px solid rgba(255,255,255,0.08)', boxShadow: '0 2px 12px rgba(0,0,0,0.4)', fontSize: 11, color: 'rgba(255,255,255,0.55)', cursor: 'pointer' }}>
            🎓 Tuto
          </button>
          <Link href="/settings" style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '8px 14px', borderRadius: 20, background: 'rgba(15,12,10,0.85)', backdropFilter: 'blur(12px)', border: '1px solid rgba(255,255,255,0.08)', boxShadow: '0 2px 12px rgba(0,0,0,0.4)', textDecoration: 'none', fontSize: 11, color: 'rgba(255,255,255,0.55)' }}>
            ⚙️
          </Link>
        </div>

        {/* ── BOTTOM NAV MOBILE ── */}
        <div style={{ position: 'fixed', bottom: 0, left: 0, right: 0, background: 'rgba(10,8,6,0.88)', backdropFilter: 'blur(20px)', borderTop: '1px solid rgba(255,255,255,0.05)', display: 'flex', overflowX: 'auto', padding: '6px 8px', gap: 2, zIndex: 40 }} className="md:hidden">
          {modules.slice(0, 6).map(mod => (
            <Link key={mod.href} href={mod.href} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '5px 8px', borderRadius: 10, textDecoration: 'none', minWidth: 52, flexShrink: 0 }}>
              ...
              <span style={{ fontSize: 9, color: 'rgba(255,255,255,0.4)', marginTop: 2 }}>{mod.title.split(' ')[0]}</span>
            </Link>
          ))}
          <button onClick={() => setShowModules(true)} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '5px 8px', borderRadius: 10, border: 'none', background: 'none', cursor: 'pointer', minWidth: 52, flexShrink: 0 }}>
            ...
            <span style={{ fontSize: 9, color: 'rgba(255,255,255,0.4)', marginTop: 2 }}>Plus</span>
          </button>
        </div>
