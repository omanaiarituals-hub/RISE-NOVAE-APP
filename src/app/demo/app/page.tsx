...
const DEMO_MODULES = [
  { id: 'programme', icon: '🎯', label: 'Programme 90j', color: '#F5EDE8', href: '/demo/app/program' },
  { id: 'planner',   icon: '📅', label: 'Planner',        color: '#E8EEF5', href: '/demo/app/planner' },
  { id: 'defis',     icon: '⚡', label: 'Défis',          color: '#F0E8F5', href: '/demo/app/defis' },
  { id: 'tracker',   icon: '📊', label: 'Tracker',        color: '#E8F5EC', href: '/demo/app/tracker' },
  { id: 'routines',  icon: '☀️', label: 'Routines',       color: '#FFF5E8', href: '/demo/app/routines' },
  { id: 'agent',     icon: '🤖', label: 'Agent IA',       color: '#F5E8E8', href: '/demo/app/agent' },
  { id: 'recettes',  icon: '🛒', label: 'Recettes',       color: '#E8F5F0', href: '/demo/app/recipes' },
  { id: 'famille',   icon: '💛', label: 'Famille',        color: '#FFF5E8', href: '/demo/app/family' },
  { id: 'notes',     icon: '📝', label: 'Notes',          color: '#F5F0E8', href: '/demo/app/notes' },
  { id: 'communaute',icon: '👥', label: 'Communauté',     color: '#EEE8F5', href: '/demo/app/community' },
]
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#FAF7F2', fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <div style={{
        ...
        {/* Lien retour accueil démo */}
        <Link href="/demo/app" style={{
        ...
        <button
          ...
          style={{ background: '#C4956A', border: 'none', borderRadius: 6, padding: '6px 12px', color: 'white', fontSize: 11, fontWeight: 700, cursor: 'pointer', flexShrink: 0, fontFamily: "'DM Sans', sans-serif" }}
        >
          Rejoindre ✦
        </button>
      </div>
      ...
      <div style={{
        ...
        <Link href="/demo/app" style={{ textDecoration: 'none' }}>
      ...
      <div style={{ padding: '20px', maxWidth: 500, margin: '0 auto' }}>
        ...
        <div style={{ background: '#1A1A1A', borderRadius: 20, padding: '20px', marginBottom: 16, position: 'relative', overflow: 'hidden' }}>
          ...
          <div style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 48, fontWeight: 400, color: '#C4956A', lineHeight: 1 }}>
            1 <span style={{ fontSize: 20, color: 'rgba(255,255,255,0.3)' }}>/90</span>
          </div>
          ...
          <Link href="/demo/app/program" style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: '#C4956A', color: 'white', padding: '10px 18px', borderRadius: 10, fontSize: 12, fontWeight: 600, textDecoration: 'none' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10, marginBottom: 16 }}>
          {[
            { icon: '📋', value: '0/0', label: 'tâches',  href: '/demo/app/planner' },
            { icon: '☀️', value: '0/5', label: 'routines', href: '/demo/app/routines' },
            { icon: '🤖', value: 'Agent', label: 'NOVAÉ',  href: '/demo/app/agent' },
          ].map((s, i) => (
            <Link key={i} href={s.href} style={{ textDecoration: 'none' }}>
              <div style={{ background: 'white', borderRadius: 14, padding: '14px 10px', textAlign: 'center', border: '1px solid rgba(196,149,106,0.1)' }}>
        ...
        {/* Grille modules */}
        <p style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.15em', textTransform: 'uppercase', color: 'rgba(26,26,26,0.3)', marginBottom: 12 }}>
          Tous les modules
        </p>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10, marginBottom: 20 }}>
          {DEMO_MODULES.map(m => (
            <Link
              key={m.id}
              href={m.href}
              style={{ background: m.color, borderRadius: 14, padding: '16px 12px', textAlign: 'center', textDecoration: 'none', display: 'block', border: '1px solid rgba(196,149,106,0.08)' }}
            >
        ...
        <div style={{ background: 'white', borderRadius: 20, padding: '20px', border: '1px solid rgba(196,149,106,0.15)', textAlign: 'center' }}>
          ...
          <button
            onClick={() => setShowConvert(true)}
            style={{ width: '100%', padding: '14px', background: '#1A1A1A', border: 'none', borderRadius: 12, color: 'white', fontSize: 13, fontWeight: 700, cursor: 'pointer', fontFamily: "'DM Sans', sans-serif", marginBottom: 8 }}
          >
            ✦ Rejoindre la bêta gratuitement
          </button>
          <button
            onClick={() => { setMode('waitlist'); setShowConvert(true) }}
            style={{ width: '100%', padding: '12px', background: 'transparent', border: '1px solid #E8E0D8', borderRadius: 12, color: '#6B6560', fontSize: 12, cursor: 'pointer', fontFamily: "'DM Sans', sans-serif" }}
          >
            📩 Liste d'attente
          </button>
        </div>
        ...
        <div style={{
          ...
          {[
            { icon: '🏠', label: 'Accueil',  href: '/demo/app' },
            { icon: '🎯', label: 'Programme', href: '/demo/app/program' },
            { icon: '📅', label: 'Planner',   href: '/demo/app/planner' },
            { icon: '☀️', label: 'Routines',  href: '/demo/app/routines' },
            { icon: '🤖', label: 'Agent',     href: '/demo/app/agent' },
          ].map((item, i) => (
            <Link key={i} href={item.href} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textDecoration: 'none', minWidth: 52 }}>
              <span style={{ fontSize: 20 }}>{item.icon}</span>
              <span style={{ fontSize: 9, color: '#6B6560', marginTop: 3, fontWeight: 500 }}>{item.label}</span>
            </Link>
          ))}
        </div>
            ...
            {mode === 'done' ? (
              <div style={{ textAlign: 'center', padding: '16px 0' }}>
                ...
                <button
                  ...
                  style={{ marginTop: 20, padding: '12px 24px', background: '#1A1A1A', border: 'none', borderRadius: 10, color: 'white', fontSize: 13, cursor: 'pointer', fontFamily: "'DM Sans', sans-serif" }}
                >
              ...
              <div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                  ...
                  <button
                    ...
                    style={{ padding: '16px', background: '#FAF7F2', border: '1.5px solid #E8E0D8', borderRadius: 12, color: '#1A1A1A', fontSize: 13, fontWeight: 600, cursor: 'pointer', textAlign: 'left', display: 'flex', alignItems: 'center', gap: 12, fontFamily: "'DM Sans', sans-serif" }}
                  >
                    <span style={{ fontSize: 22 }}>📩</span>
                    <div>
              ...
              <div>
                <button
                  ...
                  style={{ background: 'none', border: 'none', color: '#6B6560', fontSize: 12, cursor: 'pointer', marginBottom: 16, padding: 0 }}
                >
                  ← Retour
                </button>
