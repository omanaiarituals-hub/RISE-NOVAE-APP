      ...
      <div style={{ width: '100%', maxWidth: 420 }}>
        ...
        <div style={{ background: '#FFFFFF', borderRadius: 20, padding: '36px 32px', boxShadow: '0 4px 24px rgba(0,0,0,0.07)', border: '1px solid #F0EAE2' }}>
          <div style={{ textAlign: 'center', marginBottom: 24 }}>
            ...
            <h2 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 24, fontWeight: 500, color: '#1A1A1A', margin: '0 0 6px' }}>Nouveau mot de passe</h2>
            <p style={{ fontSize: 13, color: '#6B6B6B', margin: 0 }}>Choisis un nouveau mot de passe sécurisé.</p>
          </div>

          <form onSubmit={handleReset} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            <div>
              <label style={{ fontSize: 11, fontWeight: 600, color: '#6B6B6B', display: 'block', marginBottom: 6, textTransform: 'uppercase', letterSpacing: '0.08em' }}>Nouveau mot de passe</label>
              <div style={{ position: 'relative' }}>
                ...
                <button type="button" onClick={() => setShowPassword(!showPassword)}
                  style={{ position: 'absolute', right: 12, top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer', color: '#6B6B6B', fontSize: 16, padding: 4 }}>
                  {showPassword ? '🙈' : '👁️'}
                </button>
              </div>
            </div>
            ...
            <div>
              <label style={{ fontSize: 11, fontWeight: 600, color: '#6B6B6B', display: 'block', marginBottom: 6, textTransform: 'uppercase', letterSpacing: '0.08em' }}>Confirmer le mot de passe</label>
              <input
                type={showPassword ? 'text' : 'password'}
